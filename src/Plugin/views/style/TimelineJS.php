...
class TimelineJS extends StylePluginBase {
  ...
  protected function buildDate($field) {
    try {
      ...
      $this->messenger()->addMessage($this->t('The date "@date" does not 
      conform to a <a href="@php-manual">PHP supported date and time 
      format</a>.',
       [
         '@date' => $date_string,
  ...
  protected function handleXmlErrors(\LibXMLError $error, $html) {
    $message = $this->t('A media field has an error in its HTML.<br>
    Error message: @message<br>Views result row: @row<br>
    HTML: <pre>@html</pre>',
     [
